...
from .clip_krnft import CLIP_KRNFT
from .krnft_modules import migrate_legacy_state_dict
...
def get_network(network_config):
    ...
    elif network_config.name == 'clip_krnft':
        net = CLIP_KRNFT(network_config)
    ...
    if network_config.pretrained:
        if type(net) is dict:
            ...
            checkpoint_path = os.path.join(
                network_config.output_dir, 'model_checkpoint.pth'
            )
            checkpoint = torch.load(checkpoint_path, map_location='cpu')
            learner_state = migrate_legacy_state_dict(
                checkpoint['taskres_prompt_learner_state_dict']
            )
            net.model.taskres_prompt_learner.load_state_dict(
                learner_state, strict=False
            )
            return net.cuda()
        else:
            try:
