...
from openood.networks.krnft_modules import DistributionAwareFeatureTransformer
...
imagenet_r_mask = [wnid in imagenet_200_wnids for wnid in all_wnids]
imagenet200_classes = []
...
def get_text_features_neg(model, dataset, text_prompt, text_center, ood_number):
    ...
    with torch.no_grad():
        ...
        for classname in tqdm(classnames):
            texts = [template.format(classname) for template in templates]
            texts = clip.tokenize(texts).cuda()
            class_embeddings = model.encode_text(texts).to(torch.float32)
            class_embeddings /= class_embeddings.norm(dim=-1, keepdim=True)
            if text_center:
                class_embedding = class_embeddings.mean(dim=0)
                class_embedding /= class_embedding.norm()
    ...
    text_features = torch.cat((text_features, text_features_neg), dim=1).t()
...
class CscTaskResLearner(nn.Module):
    ...
    def forward(self, im_features):
        ...
        scaling_bias = scaling_bias.expand(self.classnum, -1)
        scaling_bias = scaling_bias.to(self.device)
        ex_scaling_bias = self.scaling_ex_meta_net(im_features)
        ex_scaling_bias = ex_scaling_bias.expand(self.base_text_features.shape[0]-self.classnum, -1)
        ...
        shift_bias = self.bias_meta_net(im_features)
        shift_bias = shift_bias.expand(self.classnum, -1)
        shift_bias = shift_bias.to(self.device)
        ex_shift_bias = self.bias_ex_meta_net(im_features)
        ex_shift_bias = ex_shift_bias.expand(self.base_text_features.shape[0]-self.classnum, -1)
...
class UniTaskResLearner(nn.Module):
    ...
    def forward(self, im_features):
        ...
        scaling_bias = scaling_bias.expand(self.classnum, -1)
        scaling_bias = scaling_bias.to(self.device)
        ex_scaling_bias = self.scaling_meta_net(im_features)
        ex_scaling_bias = ex_scaling_bias.expand(self.base_text_features.shape[0]-self.classnum, -1)
        ex_scaling_bias = ex_scaling_bias.to(self.device)
        ...
        shift_bias = shift_bias.expand(self.classnum, -1)
        shift_bias = shift_bias.to(self.device)
        ex_shift_bias = self.bias_meta_net(im_features)
        ex_shift_bias = ex_shift_bias.expand(self.base_text_features.shape[0]-self.classnum, -1)
        ex_shift_bias = ex_shift_bias.to(self.device)
        shift = torch.cat((shift + shift_bias, ex_shift + ex_shift_bias), dim=0)
...
class ConstantTaskResLearner(nn.Module):
    ...
    def forward(self, im_features):
        ...
        scaling_bias = self.scaling_meta_net(im_features)
        scaling_bias = scaling_bias.expand(self.classnum, 512)
        scaling_bias = scaling_bias.to(self.device)
        ex_scaling_bias = self.scaling_meta_net(im_features)
        ex_scaling_bias = ex_scaling_bias.expand(self.base_text_features.shape[0]-self.classnum, 512)
        ex_scaling_bias = ex_scaling_bias.to(self.device)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, all_text_features):
        ...
        if cfg.backbone.dis_type == 'csc':
            ...
            self.taskres_prompt_learner = DistributionAwareFeatureTransformer(
                cfg, clip_model, self.all_text_features
            )
...
class CLIP_KRNFT(nn.Module):
    """CLIP wrapper that exposes the KR-NFT text-feature adaptation."""

    def __init__(self, cfg, criterion='cosine', device='cuda', **_):
        super().__init__()
        classnames = get_class_names(cfg.backbone.dataset)
        self.n_cls = len(classnames)

        backbone_name = cfg.backbone.name
        clip_model, _ = clip.load(backbone_name, device, jit=False)
        clip_model.eval()
        self.clip_model = clip_model
        self.n_output = self.n_cls + int(cfg.backbone.OOD_NUM)

        self.image_encoder = clip_model.visual.to(torch.device(device))

        self.text_encoder = TextEncoder(clip_model)
        self.text_encoder = self.text_encoder.to(torch.device(device))
        self.logit_scale = clip_model.logit_scale.data

        self.criterion = criterion
        text_center = cfg.backbone.text_center
        self.text_features, self.all_text_features_unselected = get_text_features_neg(
            clip_model,
        ...
        self.ood_count = int(cfg.backbone.OOD_NUM)
    ...
    def _get_base_text_features(self, classnames, clip_model, text_encoder):
        ...
        with torch.no_grad():
            ...
            for text in classnames:
                tokens = clip.tokenize([template.format(text) for template in TEMPLATES]).cuda()
